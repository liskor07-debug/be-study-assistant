...
cv_intra — коэффициент внутрииндивидуальной вариации (В %!)
...
cmax — максимальная концентрация (в нг/мл или мкг/л)
...
auc — площадь под кривой (в нг·ч/мл или мкг·ч/л)
...
tmax — время достижения Cmax (В ЧАСАХ!)
...
ВАЖНЫЕ ПРАВИЛА:
Все значения должны быть числами (float), не строками
Если параметр не найден — укажи null (не 0, не "N/A")
Конвертируй единицы: минуты → часы (раздели на 60), дни → часы (умножь на 24)
Если найдено несколько значений — возьми среднее или наиболее релевантное для однократной дозы
Ищи в любых разделах: аннотация, методы, результаты, таблицы, выводы
Обращай внимание на контекст — параметры должны относиться к препарату {inn}
Текст для анализа:
{text}...
...
if run_button:
    ...
    with st.spinner("Поиск и анализ данных..."):
        if use_auto_pk:
            ...
            studies = pk_data.get("studies", []) if pk_data else []
